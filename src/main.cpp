...
const string strMessageMagic = "Xarr Signed Message:\n";
...
{
    // Xarr: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 1850000); // Xarr: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 30240; // Xarr: 3.5 days
static const int64 nTargetSpacing = 30; // Xarr: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Xarr: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Xarr: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Xarr: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x8b69e5bd3bc34c0e65b3145e9d9d9b690f319427389e3f19189a1ef960f3f00a");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        block.nVersion = 1;
        block.nTime    = 1524531058;
        block.nBits    = 503421396;
        block.nNonce   = 1865299171;

        if (fTestNet)
        {
            block.nTime    = 1524531058;
            block.nNonce   = 1955718210;
        }
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Xarr: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("XarrMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("XarrMiner : ProcessBlock, block not accepted");
    }
